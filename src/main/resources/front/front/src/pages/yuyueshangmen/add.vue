...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"auto","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="预约备注" prop="yuyuebeizhu">
            <el-input  v-model="ruleForm.yuyuebeizhu" 
                placeholder="预约备注" clearable :disabled=" false  ||ro.yuyuebeizhu"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yuyueshangmen/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yuyueshangmen/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yuyueshangmen/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
