...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 6px","margin":"172px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"right","justifyContent":"space-between","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"380px","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>社区名称：</div>
        <el-input v-model="formSearch.shequmingcheng" placeholder="社区名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0","margin":"0px 0 0","borderRadius":"0px","background":"none","width":"calc(100% - 400px)","float":"left","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"31.3%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"480px"}' class="item">
			<img @click.stop="imgPreView(item.fengmian)" v-if="item.fengmian && item.fengmian.substr(0,4)=='http'" :src="item.fengmian.split(',')[0]" class="image" />
			...
			<div class="content">
		      <div :style='{"padding":"30% 10px 20px 0","borderColor":"#ddd","color":"inherit","alignItems":"center","display":"block","textIndent":"2em","overflow":"hidden","borderWidth":"0 1px 0 0","width":"48%","lineHeight":"30px","fontSize":"inherit","borderStyle":"dashed","height":"100%"}'>{{item.fuwujianjie}}</div>
		      <div class="info" :style='{"padding":"30% 0 0 0px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","display":"block","width":"48%","height":"100%"}'>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.shequmingcheng}}</div>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.fuwufenlei}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('fuwufenlei/list',{}).then(res => {
		  if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.shequmingcheng != '') searchWhere.shequmingcheng = '%' + this.formSearch.shequmingcheng + '%';
        if (this.curFenlei != '全部') searchWhere.fuwufenlei = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`shequfuwu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/shequfuwuDetail', query: params});
      },
