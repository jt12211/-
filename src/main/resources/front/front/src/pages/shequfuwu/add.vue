...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"auto","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="服务分类" prop="fuwufenlei">
            <el-select v-model="ruleForm.fuwufenlei" placeholder="请选择服务分类"  >
              <el-option
          ...
          <el-form-item :style='{"width":"auto","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务简介" prop="fuwujianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="服务简介"
          ...
          <el-form-item :style='{"width":"auto","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务详情" prop="fuwuxiangqing">
            <editor 
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/fuwufenlei/fuwufenlei', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.fuwufenleiOptions = res.data.data;
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shequfuwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shequfuwu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shequfuwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
