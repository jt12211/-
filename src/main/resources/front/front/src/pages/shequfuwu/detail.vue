...
<div>
	<div :style='{"padding":"16px 20px","margin":"16px auto","borderColor":"#eee","borderRadius":"30px","background":"#ffffff","borderWidth":"0px","width":"86%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"16px","lineHeight":"1","justifyContent":"flex-start","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/shequfuwu"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"0 7%","margin":"40px auto","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<div class="attr" :style='{"minHeight":"600px","padding":"10px 0 20px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"40%","position":"relative","justifyContent":"space-between","order":"1"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"0px","margin":"0","background":"#fff","flex":"1","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0 10px","margin":"0 0 4px 0","borderColor":"#28890b10","background":"none","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>发布时间</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#333","flex":"1","height":"auto"}'>{{detail.fabushijian}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10px","margin":"0 0 4px 0","borderColor":"#28890b10","background":"none","borderWidth":"0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>服务简介</div>
					<div  :style='{"padding":"8px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#333","flex":"1","height":"auto"}'>{{detail.fuwujianjie}}</div>
				</div>
				<div class="btn" :style='{"width":"450px","padding":"10px 30px","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"6px","background":"#1c6a6d","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('shequfuwu','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 10px 10px 0","outline":"none","color":"#fff","borderRadius":"6px","background":"#86be9c","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('shequfuwu','删除')" @click="delClick">删除</el-button>
					<!-- hasChat 无 -->
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"6px","background":"#1e3c4f","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('shequfuwu','预约上门')" @click="onAcross('yuyueshangmen','','','')" type="warning">预约上门</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      onAcross(acrossTable,crossOptAudit,statusColumnName,tips,statusColumnValue){
        localStorage.setItem('crossTable',`shequfuwu`);
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('shequfuwu/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'shequfuwu', userid: localStorage.getItem('frontUserid')}}).then(res => {
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/shequfuwuAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此社区服务？')
			  .then(_ => {
			    this.$http.post('shequfuwu/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
