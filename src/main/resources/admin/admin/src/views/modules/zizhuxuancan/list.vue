		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","background":"#fff"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('zizhuxuancan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"margin":"0 2px","fontSize":"14px","color":"#0E9BD3","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zizhuxuancan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zizhuxuancan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('zizhuxuancan','下单')" @click="dingdanxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								下单
							</el-button>
							<el-button class="edit" v-if=" isAuth('zizhuxuancan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								编辑
							</el-button>

							<el-button class="view" v-if="isAuth('zizhuxuancan','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('zizhuxuancan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import dingdanxinxiCrossAddOrUpdate from "../dingdanxinxi/add-or-update";
	export default {
		...
		methods: {
			...
			dingdanxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','zizhuxuancan');
	...
	this.$router.push({path:'/discusszizhuxuancan',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	zizhuxuancanstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zizhuxuancan/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
