	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="服务分类" prop="fuwufenlei">
					<el-select :disabled="ro.fuwufenlei" v-model="ruleForm.fuwufenlei" placeholder="请选择服务分类" >
						<el-option
							v-for="(item,index) in fuwufenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="服务分类" prop="fuwufenlei">
					<el-input v-model="ruleForm.fuwufenlei"
						placeholder="服务分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="服务简介" prop="fuwujianjie">
					<el-input
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.fuwujianjie" label="服务简介" prop="fuwujianjie">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.fuwujianjie}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="服务详情" prop="fuwuxiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.fuwuxiangqing" label="服务详情" prop="fuwuxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.fuwuxiangqing"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fuwujianjie'){
							this.ruleForm.fuwujianjie = obj[o];
							this.ro.fuwujianjie = true;
							continue;
						}
						if(o=='fuwuxiangqing'){
            ...
            this.$http({
				url: `option/fuwufenlei/fuwufenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.fuwufenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.fuwuxiangqing = this.ruleForm.fuwuxiangqing.replace(reg,'../../../springbootfuca0297/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shequfuwu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shequfuwuCrossAddOrUpdateFlag = false;
						 this.parent.search();
