	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="反馈标题" prop="fankuibiaoti">
					<el-input  v-model="ruleForm.fankuibiaoti" placeholder="反馈标题" clearable  :readonly="ro.fankuibiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="反馈标题" prop="fankuibiaoti">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="反馈时间" prop="fankuishijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.fankuishijian" label="反馈时间" prop="fankuishijian">
					<el-input v-model="ruleForm.fankuishijian" placeholder="反馈时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="反馈内容" prop="fankuineirong">
					<editor 
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.fankuineirong" label="反馈内容" prop="fankuineirong">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.fankuineirong"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.fankuineirong = this.ruleForm.fankuineirong.replace(reg,'../../../springbootfuca0297/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `suqiufankui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.suqiufankuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
