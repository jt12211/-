<template>
	<div class="main-content" :style='{"width":"calc(100% - 60px)","padding":"30px","margin":"0 30px","background":"#fff"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#9E9E9E","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">社区名称</label>
						<el-input v-model="searchForm.shequmingcheng" placeholder="社区名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuyueshangmen','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"margin":"0 2px","fontSize":"14px","color":"#0E9BD3","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yuyueshangmen','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyueshangmen','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueshangmen','服务评价')" @click="fuwupingjiaCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								服务评价
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyueshangmen','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yuyueshangmen','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<fuwupingjia-cross-add-or-update v-if="fuwupingjiaCrossAddOrUpdateFlag" :parent="this" ref="fuwupingjiaCrossaddOrUpdate"></fuwupingjia-cross-add-or-update>
...
import fuwupingjiaCrossAddOrUpdate from "../fuwupingjia/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				fuwupingjiaCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fuwupingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fuwupingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyueshangmen');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fuwupingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fuwupingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.shequmingcheng!='' && this.searchForm.shequmingcheng!=undefined){
            params['shequmingcheng'] = '%' + this.searchForm.shequmingcheng + '%'
          }
	...
	yuyueshangmenstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'yuyueshangmen/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
