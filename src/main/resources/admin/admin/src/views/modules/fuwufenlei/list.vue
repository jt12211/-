			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#9E9E9E","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">服务分类</label>
						<el-input v-model="searchForm.fuwufenlei" placeholder="服务分类" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fuwufenlei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"margin":"0 2px","fontSize":"14px","color":"#0E9BD3","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('fuwufenlei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('fuwufenlei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('fuwufenlei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.fuwufenlei!='' && this.searchForm.fuwufenlei!=undefined){
	...
	fuwufenleistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'fuwufenlei/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
