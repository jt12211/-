			...
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#9E9E9E","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">医院名称</label>
						<el-input v-model="searchForm.yiyuanmingcheng" placeholder="医院名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#9E9E9E","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">医生姓名</label>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yiliaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"margin":"0 2px","fontSize":"14px","color":"#0E9BD3","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yiliaoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yiliaoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('yiliaoxinxi','预约挂号')" @click="yuyueguahaoCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								预约挂号
							</el-button>
							<el-button class="edit" v-if=" isAuth('yiliaoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								编辑
							</el-button>

							<el-button class="view" v-if="isAuth('yiliaoxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yiliaoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import yuyueguahaoCrossAddOrUpdate from "../yuyueguahao/add-or-update";
	export default {
		...
		methods: {
			...
			yuyueguahaoCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','yiliaoxinxi');
	...
	this.$router.push({path:'/discussyiliaoxinxi',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	yiliaoxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'yiliaoxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
